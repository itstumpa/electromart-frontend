...
import { ChevronDown, ExternalLink, Eye, EyeOff, Star, X } from "lucide-react";
...
export default function ProductsClient({
  ...
  const handleTogglePublish = async () => {
    ...
    try {
      ...
      setProducts((prev) =>
        prev.map((p) =>
          p.id === toggleTarget.id ? { ...p, isPublished: !p.isPublished } : p,
        ),
      );
      toast.success(
        toggleTarget.isPublished ? "Product hidden" : "Product published",
      );
    } catch (err) {
  ...
  const columns: Column<Product>[] = [
    ...
    },
    {
      ...
      render: (p) => (
        <span
          className={`text-sm font-bold ${p.stock <= 10 ? "text-red-600" : p.stock <= 30 ? "text-yellow-600" : "text-green-700"}`}
        >
          {p.stock}
        </span>
      ),
    },
    {
      ...
      render: (p) => (
        <span
          className={`inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-full ${p.isPublished ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}
        >
          <span
            className={`w-1.5 h-1.5 rounded-full ${p.isPublished ? "bg-green-500" : "bg-slate-400"}`}
          />
  ...
  const actions = (p: Product) => (
    <div className="flex items-center justify-end gap-2">
      ...
      <button
        ...
        className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-blue-100 text-slate-500 hover:text-blue-700 flex items-center justify-center transition-colors"
        title="Details"
      >
        <Eye size={14} />
      </button>
  ...
  return (
    <>
      ...
      <AdminDataTable
        data={filtered}
        columns={columns}
        searchKeys={["name", "storeName", "sku"]}
        searchPlaceholder="Search by name, store or SKU..."
        actions={actions}
      ...
      <ConfirmModal
        ...
        title={
          toggleTarget?.isPublished
            ? `Hide ${toggleTarget?.name}?`
            : `Publish ${toggleTarget?.name}?`
        }
        description={
          toggleTarget?.isPublished
        ...
        confirmLabel={
          toggleTarget?.isPublished ? "Hide Product" : "Publish Product"
        }
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <img
                ...
                alt={viewTarget.name}
                className="w-full h-44 object-contain bg-slate-50 rounded-xl mb-5"
              />
              ...
              <div className="space-y-2">
                {[
                  ...
                  { label: "Store", value: viewTarget.storeName },
                  { label: "Category", value: viewTarget.categoryName },
                  {
                    label: "Price",
                    value: `$${viewTarget.price.toLocaleString()}`,
                  },
                  { label: "Stock", value: viewTarget.stock },
                  {
                    label: "Rating",
                    value: `${viewTarget.rating} (${viewTarget.reviewCount} reviews)`,
                  },
                  {
                    label: "Featured",
                    value: viewTarget.featured ? "Yes" : "No",
                  },
                  {
                    label: "Published",
                    value: viewTarget.isPublished ? "Yes" : "No",
                  },
                ].map(({ label, value }) => (
                  <div
                    key={label}
