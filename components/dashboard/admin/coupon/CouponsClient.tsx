...
import {
  Plus, Pencil, Trash2, X, Tag, ToggleLeft, ToggleRight,
  Percent, DollarSign, Calendar, Hash, TrendingUp, Eye,
  Copy, CheckCircle2, Search, Filter,
} from 'lucide-react';
...
const MOCK_COUPONS: Coupon[] = [
  {
    id: '1', code: 'SAVE20', discountType: 'PERCENTAGE', discountValue: 20,
    minOrderAmount: 100, maxDiscount: 50, usageLimit: 100, usedCount: 34,
    expiryDate: '2026-12-31', isActive: true, createdAt: '2026-01-01T00:00:00Z',
  },
  {
    id: '2', code: 'FLAT15', discountType: 'FIXED', discountValue: 15,
    minOrderAmount: 50, usageLimit: 200, usedCount: 89,
    expiryDate: '2026-09-30', isActive: true, createdAt: '2026-02-01T00:00:00Z',
  },
  {
    id: '3', code: 'WELCOME10', discountType: 'PERCENTAGE', discountValue: 10,
    usageLimit: 500, usedCount: 412, isActive: false, createdAt: '2026-03-01T00:00:00Z',
  },
  {
    id: '4', code: 'SUMMER30', discountType: 'PERCENTAGE', discountValue: 30,
    minOrderAmount: 200, maxDiscount: 80, usageLimit: 50, usedCount: 50,
    expiryDate: '2026-08-31', isActive: false, createdAt: '2026-04-01T00:00:00Z',
  },
];
...
function CouponModal({
  ...
  const [form, setForm] = useState<CouponFormData>(initial ? couponToForm(initial) : EMPTY_FORM);
  ...
  const fields: { key: keyof CouponFormData; label: string; placeholder: string; type?: string; optional?: boolean }[] = [
    { key: 'discountValue', label: form.discountType === 'PERCENTAGE' ? 'Discount Value (%)' : 'Discount Value ($)', placeholder: form.discountType === 'PERCENTAGE' ? 'e.g. 20' : 'e.g. 15' },
    { key: 'minOrderAmount', label: 'Minimum Order Amount ($)', placeholder: 'e.g. 100', optional: true },
    ...(form.discountType === 'PERCENTAGE' ? [{ key: 'maxDiscount' as keyof CouponFormData, label: 'Maximum Discount ($)', placeholder: 'e.g. 50', optional: true }] : []),
    { key: 'usageLimit', label: 'Usage Limit', placeholder: 'e.g. 100 (unlimited if empty)', optional: true },
    { key: 'expiryDate', label: 'Expiry Date', placeholder: '', type: 'date', optional: true },
  ];
          ...
          >
            ...
            <div className="space-y-4">
              ...
              {fields.map(({ key, label, placeholder, type, optional }) => (
                <div key={key}>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wide block mb-1.5">
                    {label} {optional && <span className="text-slate-400 normal-case font-normal">(optional)</span>}
                  </label>
                  <input
                    type={type || 'number'}
              ...
              <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">Inactive coupons cannot be applied</p>
                </div>
...
/* ── View Modal ─────────────────────────────────────── */
function ViewModal({ coupon, onClose }: { coupon: Coupon; onClose: () => void }) {
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-6">
            <div className="flex-1 flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
              ...
              <span className="font-mono font-black text-amber-800 text-lg tracking-wider">{coupon.code}</span>
            </div>
            <button onClick={copy}
          ...
          <div className="space-y-3">
            {[
              { label: 'Discount', value: coupon.discountType === 'PERCENTAGE' ? `${coupon.discountValue}%` : `$${coupon.discountValue}` },
              { label: 'Type', value: coupon.discountType === 'PERCENTAGE' ? 'Percentage' : 'Fixed Amount' },
              ...(coupon.minOrderAmount != null ? [{ label: 'Min Order', value: `$${coupon.minOrderAmount}` }] : []),
              ...(coupon.maxDiscount != null ? [{ label: 'Max Discount', value: `$${coupon.maxDiscount}` }] : []),
              ...(coupon.expiryDate ? [{ label: 'Expires', value: new Date(coupon.expiryDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) }] : []),
              { label: 'Status', value: coupon.isActive ? 'Active' : 'Inactive' },
              { label: 'Created', value: new Date(coupon.createdAt).toLocaleDateString() },
            ].map(({ label, value }) => (
              <div key={label} className="flex items-center justify-between py-2.5 border-b border-slate-50">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wide">{label}</span>
                <span className={`text-sm font-bold ${
                  label === 'Status'
                    ? value === 'Active' ? 'text-green-600' : 'text-slate-400'
                    : 'text-slate-900'
                }`}>{value}</span>
              </div>
            ))}
...
function StatusBadge({ coupon }: { coupon: Coupon }) {
  ...
  if (isExpired) return <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-slate-100 text-slate-500">Expired</span>;
  if (isExhausted) return <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-orange-100 text-orange-600">Exhausted</span>;
  if (coupon.isActive) return <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-green-100 text-green-700">Active</span>;
  return <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-slate-100 text-slate-500">Inactive</span>;
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Total Coupons', value: stats.total, icon: Tag, color: 'bg-amber-100 text-amber-700' },
          { label: 'Active', value: stats.active, icon: TrendingUp, color: 'bg-green-100 text-green-700' },
          { label: 'Total Used', value: stats.totalUsed, icon: Hash, color: 'bg-blue-100 text-blue-700' },
      ...
      <div className="flex items-center gap-3 flex-wrap">
          ...
          {(['ALL', 'ACTIVE', 'INACTIVE'] as const).map((f) => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold capitalize transition-all ${
                filter === f ? 'bg-amber-600 text-white' : 'text-slate-500 hover:text-slate-900'
              }`}>
              {f}
            </button>
          ))}
        ...
        <button
          onClick={() => { setEditCoupon(null); setModal(true); }}
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5">
                      <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-lg ${
                        ...
                        {coupon.discountType === 'PERCENTAGE' ? <Percent size={10} /> : <DollarSign size={10} />}
                    ...
                    <td className="px-4 py-3.5">
                      ...
                      {coupon.maxDiscount && (
                        <p className="text-[10px] text-slate-400 mt-0.5">max ${coupon.maxDiscount}</p>
                      )}
                    </td>

                    {/* Min order */}
                    <td className="px-4 py-3.5 text-sm text-slate-600">
                      {coupon.minOrderAmount ? `$${coupon.minOrderAmount}` : <span className="text-slate-300">—</span>}
                    </td>
                          ...
                          <div className="w-16 bg-slate-100 rounded-full h-1.5">
                            <div
                              className={`h-1.5 rounded-full ${
                                (coupon.usedCount / coupon.usageLimit) >= 1 ? 'bg-red-500' :
                                (coupon.usedCount / coupon.usageLimit) >= 0.8 ? 'bg-amber-500' : 'bg-green-500'
                              }`}
                    ...
                    <td className="px-4 py-3.5">
                      {coupon.expiryDate ? (
                        <span className={`text-xs font-semibold flex items-center gap-1 ${
                          ...
                          <Calendar size={11} />
                          {new Date(coupon.expiryDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: '2-digit' })}
                        </span>
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-1">
                        <button onClick={() => setViewCoupon(coupon)}
                        ...
                        <button onClick={() => { setEditCoupon(coupon); setModal(true); }}
      ...
      {viewCoupon && <ViewModal coupon={viewCoupon} onClose={() => setViewCoupon(null)} />}
      ...
      <AnimatePresence>
        {deleteId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={() => setDeleteId(null)} />
            <motion.div
            ...
            >
              <h3 className="text-base font-black text-slate-900 mb-2">Delete Coupon?</h3>
              <p className="text-sm text-slate-500 mb-5">
                This will permanently delete the coupon <span className="font-mono font-bold text-slate-800">{coupons.find(c => c.id === deleteId)?.code}</span>. This action cannot be undone.
              </p>
