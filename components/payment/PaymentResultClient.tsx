    ...
    return status === "success" ? (
      <div className="bg-white rounded-3xl border border-slate-100 shadow-xl p-8 text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Link
            ...
            className="inline-flex items-center justify-center gap-2 bg-amber-600 hover:bg-amber-700 text-white font-bold px-6 py-3 rounded-xl text-sm"
          >
            View My Orders <ArrowRight size={15} />
          </Link>
          <Link
            href="/products"
            className="inline-flex items-center justify-center gap-2 border-2 border-slate-200 text-slate-700 font-bold px-6 py-3 rounded-xl text-sm"
          >
            Continue Shopping
          </Link>
        </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-xl p-8 text-center">
        ...
        >
          <XCircle
            size={32}
            className={status === "fail" ? "text-red-600" : "text-slate-500"}
          />
        </div>
        <h1
          className="text-xl font-black text-slate-900 mb-2"
      ...
      <div className="space-y-4">
        <div className="bg-white rounded-3xl border border-red-100 shadow-xl p-6 sm:p-8 text-center">
          ...
          <p className="text-slate-500 text-sm mb-3 leading-relaxed">
            Your payment could not be processed.{" "}
            <strong>No charges were made.</strong> Your order is saved and you
            can retry.
          </p>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5">
        ...
        <div className="space-y-2">
          {[
            {
              icon: "🛒",
              text: "Your order is saved in a pending state — nothing is lost.",
            },
            {
              icon: "⏰",
              text: "Pending orders are held for 24 hours before being cancelled automatically.",
            },
            {
              icon: "💳",
              text: "You can complete payment anytime from your Orders page.",
            },
          ].map(({ icon, text }) => (
            <div
              key={text}
              className="flex items-start gap-2.5 text-xs text-amber-800"
            >
