...
const TYPE_CONFIG: Record<NotificationType, { icon: React.ElementType; color: string; bg: string }> = {
  order:     { icon: ShoppingBag, color: 'text-amber-700',  bg: 'bg-amber-100' },
  review:    { icon: Package,     color: 'text-purple-700', bg: 'bg-purple-100' },
  system:    { icon: Bell,        color: 'text-blue-700',   bg: 'bg-blue-100' },
  promotion: { icon: Tag,         color: 'text-rose-600',   bg: 'bg-rose-100' },
  delivery:  { icon: Truck,       color: 'text-green-700',  bg: 'bg-green-100' },
};
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-black text-slate-900" style={{ fontFamily: "'Georgia', serif" }}>
            Notifications
            {unread > 0 && (
              <span className="ml-2 text-sm bg-red-500 text-white font-black px-2 py-0.5 rounded-full align-middle">{unread}</span>
            )}
          </h1>
          <p className="text-sm text-slate-400 mt-0.5">{notifs.length} total · {unread} unread</p>
        </div>
      ...
      <div className="flex gap-2 overflow-x-auto scrollbar-none pb-1">
        {[
          ...
          <button key={key} onClick={() => setFilter(key as NotificationType | '')}
            className={`shrink-0 text-xs font-bold px-3.5 py-2 rounded-xl transition-all ${
              filter === key ? 'bg-amber-600 text-white shadow-sm' : 'bg-white border border-slate-200 text-slate-600 hover:border-amber-300'
            }`}>
            {label}
          </button>
        ))}
              ...
              return (
                <motion.div
                  ...
                  className={`flex items-start gap-4 p-4 rounded-2xl border cursor-pointer transition-all group ${
                    n.isRead ? 'bg-white border-slate-100 hover:border-slate-200' : 'bg-amber-50 border-amber-200 hover:border-amber-300'
                  }`}
                >
                  {/* Icon */}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <p className={`text-sm font-bold leading-snug ${n.isRead ? 'text-slate-700' : 'text-slate-900'}`}>{n.title}</p>
                      <div className="flex items-center gap-1.5 shrink-0">
                        {!n.isRead && <div className="w-2 h-2 rounded-full bg-amber-500 shrink-0" />}
