...
import type { Order, Notification, WishlistItem } from '@/data/types';
...
export default function CustomerOverviewClient() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([meRes, ordersRes, notifRes, wishRes, reviewsRes]) => {
        const me = meRes.data.data;
        if (me) {
          setName(me.name);
          setEmail(me.email);
        }
        setOrders(mapOrdersToUi(ordersRes.data.data ?? []));
        setNotifications((notifRes.data.data ?? []).map(mapNotificationDtoToUi));
        setWishlist(mapWishlistItemsToUi(wishRes.data.data ?? []));
  ...
  const stats = [
    ...
    { label: 'Wishlist Items', value: wishlist.length, icon: Heart, color: 'bg-rose-100', text: 'text-rose-600', href: '/dashboard/customer/wishlist' },
    { label: 'My Reviews', value: reviewCount, icon: Star, color: 'bg-purple-100', text: 'text-purple-700', href: '/dashboard/customer/reviews' },
  ];
