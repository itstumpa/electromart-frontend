...
import {
  ...
  Pencil, Check, X, Search, ChevronDown, RotateCcw,
} from 'lucide-react';
...
export default function VendorInventoryClient() {
  ...
  const stockStatus = (stock: number) => {
    ...
    if (stock <= 5)    return { label: 'Critical',      color: 'bg-red-100 text-red-700',     dot: 'bg-red-500',    bar: 'bg-red-500',    pct: Math.min(100, (stock / 100) * 100) };
    if (stock <= 10)   return { label: 'Low Stock',     color: 'bg-orange-100 text-orange-700', dot: 'bg-orange-500', bar: 'bg-orange-500', pct: Math.min(100, (stock / 100) * 100) };
    if (stock <= 30)   return { label: 'Medium',        color: 'bg-yellow-100 text-yellow-700', dot: 'bg-yellow-500', bar: 'bg-yellow-500', pct: Math.min(100, (stock / 100) * 100) };
    return               { label: 'In Stock',       color: 'bg-green-100 text-green-700',   dot: 'bg-green-500',  bar: 'bg-green-500',  pct: Math.min(100, (stock / 100) * 100) };
  };
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/60">
                {['Product', 'SKU', 'Category', 'Stock', 'Status', 'Stock Level', 'Edit'].map((h) => (
            ...
            <tbody className="divide-y divide-slate-50">
                  ...
                  return (
                    <motion.tr key={p.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                      ...
                      <td className="px-5 py-3.5">
                        <span className="text-xs font-mono text-slate-500 bg-slate-100 px-2 py-1 rounded-lg">{p.sku}</span>
                      </td>
                      {/* Category */}
                      <td className="px-5 py-3.5">
                        <span className="text-xs font-medium text-slate-600">{p.categoryName}</span>
                      </td>
                      {/* Stock — inline edit */}
                      <td className="px-5 py-3.5">
                        {editId === p.id ? (
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={7} className="py-16 text-center text-slate-400 text-sm">No products match your filter</td></tr>
              )}
            </tbody>
          </table>
        </div>
