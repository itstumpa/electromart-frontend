...
export default function VendorSettingsClient() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-black text-slate-900" style={{ fontFamily: "'Georgia', serif" }}>Settings</h1>
        <p className="text-sm text-slate-400 mt-0.5">Manage your vendor account preferences</p>
      </div>
            ...
            <motion.div key={tab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.18 }}>
              ...
              {tab === 'account' && (
                <div className="space-y-5">
                  <h2 className="font-black text-slate-900 mb-4">Account Settings</h2>

                  <div className="grid sm:grid-cols-2 gap-4">
                    {[
                      { label: 'Business / Legal Name', k: 'businessName', placeholder: 'TechStore Pro Ltd.' },
                      { label: 'Tax ID / VAT Number',   k: 'taxId',        placeholder: 'TX-2024-XXXXX' },
                      { label: 'Min Payout Amount ($)', k: 'minPayout',    placeholder: '100' },
                      ...
                      <div key={k}>
                        ...
                        <input type="text" placeholder={placeholder} value={(account as any)[k]}
                          onChange={(e) => setAccount({ ...account, [k]: e.target.value })}
                    ...
                    <div>
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-widest block mb-1.5">Payout Cycle</label>
                      <select value={account.payoutCycle} onChange={(e) => setAccount({ ...account, payoutCycle: e.target.value })}
                        className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent cursor-pointer">
                  ...
                  <div className="bg-slate-50 rounded-2xl px-5 divide-y divide-slate-100">
                    <Toggle label="Auto-accept Orders"   sub="Automatically confirm new orders without manual review"    value={account.autoAcceptOrders} onChange={(v) => setAccount({ ...account, autoAcceptOrders: v })} />
                    <Toggle label="Auto-update Stock"    sub="Reduce stock count automatically when orders are confirmed" value={account.autoUpdateStock}  onChange={(v) => setAccount({ ...account, autoUpdateStock: v })} />
                  </div>

                  <div className="flex justify-end pt-2"><SaveButton saving={saving} saved={saved} onClick={save} /></div>
                </div>
              )}
              ...
              {tab === 'notifications' && (
                <div className="space-y-6">
                  ...
                  {[
                    {
                      title: 'Orders', icon: Package, items: [
                        { k: 'newOrder',       label: 'New Order',        sub: 'Alert when a customer places an order' },
                        { k: 'orderCancelled', label: 'Order Cancelled',  sub: 'Alert when a customer cancels an order' },
                        { k: 'returnRequest',  label: 'Return Request',   sub: 'Alert when a return is submitted' },
                      ]
                    },
                    {
                      title: 'Store', icon: Store, items: [
                        { k: 'lowStock',       label: 'Low Stock Alert',  sub: 'Alert when any product falls below 10 units' },
                        { k: 'newReview',      label: 'New Review',       sub: 'Alert when a customer writes a review' },
                        { k: 'payoutSent',     label: 'Payout Sent',      sub: 'Alert when your payout is processed' },
                      ]
                    },
                    {
                      title: 'Marketing', icon: Mail, items: [
                        { k: 'weeklyReport',   label: 'Weekly Report',    sub: 'Summary of store performance every Monday' },
                        { k: 'marketingTips',  label: 'Marketing Tips',   sub: 'Seller tips and growth insights from ElectroMart' },
                      ]
                    },
                    ...
                    <div key={title}>
                      ...
                      <div className="bg-white rounded-2xl border border-slate-100 px-5 divide-y divide-slate-50">
                        {items.map(({ k, label, sub }) => (
                          <Toggle key={k} label={label} sub={sub}
                      ...
                      <div className="flex gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 mt-0.5 leading-relaxed max-w-xs">
                            Protect your vendor account with an extra layer of security.
                          </p>
