...
export default function VendorOverviewPage() {
  ...
  useEffect(() => {
    Promise.all([getMyStore(), getMyAnalytics()])
      ...
      .catch((err) => toast.error(getApiErrorMessage(err, 'Failed to load dashboard')))
  ...
  if (!store) {
    return (
      <div className="text-center py-20">
        ...
        <Link href="/dashboard/vendor/store" className="mt-3 text-sm text-amber-600 font-bold hover:underline">
          Create your store
        </Link>
      </div>
    );
  ...
  const totalRevenue = analytics?.monthlyRevenue.reduce((s, d) => s + d.revenue, 0) ?? 0;
  const totalOrders  = Object.values(analytics?.ordersByStatus ?? {}).reduce((s, v) => s + v, 0);
  ...
  const stats = [
    { label: 'Total Revenue', value: `$${(totalRevenue / 1000).toFixed(1)}k`, icon: Wallet,      bg: 'bg-amber-100',  text: 'text-amber-700',  change: `${store.totalSales} sales` },
    { label: 'Total Orders',  value: totalOrders,                              icon: ShoppingBag, bg: 'bg-blue-100',   text: 'text-blue-700',   change: `${analytics?.totalCustomers ?? 0} customers` },
    { label: 'Products',      value: store.products.length,                   icon: Package,     bg: 'bg-purple-100', text: 'text-purple-700', change: `${lowStock.length} low stock` },
    { label: 'Store Rating',  value: `${store.rating}★`,                      icon: Star,        bg: 'bg-green-100',  text: 'text-green-700',  change: `avg $${analytics?.averageOrderValue ?? 0}` },
  ];
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${store.isApproved ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                {store.isApproved ? '✓ Approved' : '⏳ Pending'}
              </span>
              <span className="text-xs text-slate-400">
                Since {new Date(store.createdAt).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}
              </span>
            </div>
          </div>
        </div>
        ...
        {stats.map(({ label, value, icon: Icon, bg, text, change }) => (
          <div key={label} className="bg-white rounded-2xl border border-slate-100 p-4 sm:p-5 flex flex-col gap-3 hover:shadow-md hover:border-amber-200 transition-all">
            <div className="flex items-start justify-between">
              ...
              <span className="text-[10px] font-bold text-slate-500 bg-slate-50 px-2 py-0.5 rounded-full">{change}</span>
            </div>
            <div>
            ...
            {analytics.topProducts.map((p, i) => (
              <div key={p.id} className="flex items-center gap-3 px-5 py-3.5 hover:bg-slate-50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400">{p.totalSold} sold · ${Number(p.price).toLocaleString()}</p>
                </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {Object.entries(analytics.ordersByStatus).map(([status, count]) => {
              const s = STATUS_CONFIG[status] ?? { label: status, color: 'bg-slate-100 text-slate-600', dot: 'bg-slate-400' };
              return (
