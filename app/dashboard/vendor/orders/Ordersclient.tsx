...
import { getVendorOrders, type VendorOrderItemDto } from '@/api/order.api';
...
const STATUS_OPTIONS: ItemStatus[] = ['PENDING', 'PROCESSING', 'SHIPPED', 'DELIVERED', 'CANCELLED'];
...
useEffect(() => {
  ...
  const updateStatus = async (itemId: string, status: ItemStatus) => {
    setUpdating(itemId);
    try {
      await api.patch<ApiResponse<null>>(`/orders/vendor/items/${itemId}/status`, { status });
      setItems((prev) => prev.map((i) => i.id === itemId ? { ...i, status } : i));
      if (viewItem?.id === itemId) setViewItem((i) => i ? { ...i, status } : null);
  ...
  const revenue   = items.reduce((s, i) => s + Number(i.priceAtTime) * i.quantity, 0);
    ...
    <>
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-0.5">
              {items.length} orders · ${revenue.toLocaleString('en-US', { minimumFractionDigits: 0 })} revenue
            </p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: 'Total Orders', value: items.length,  color: 'text-slate-900',  bg: 'bg-white' },
                  ...
                  <div className="flex items-center gap-3 px-4 sm:px-5 py-4 flex-wrap">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="text-sm font-black text-amber-600">#{item.orderId.slice(-6).toUpperCase()}</p>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5 truncate">
                        {item.order.user.name} · {item.product.name} · qty {item.quantity} ·{' '}
                        ...
                        <div className="absolute right-0 top-full mt-1 w-44 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden z-30">
                          {STATUS_OPTIONS.map((st) => {
                            ...
                            return (
                              <button
                                key={st}
                                onClick={() => { updateStatus(item.id, st); setOpenDropdown(null); }}
        ...
        {viewItem && (
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm" onClick={() => setViewItem(null)} />
            <motion.div
              ...
              <div className="p-5 sm:p-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <h3 className="text-lg font-black text-slate-900">#{viewItem.orderId.slice(-6).toUpperCase()}</h3>
                  </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="relative">
                    <select
                      ...
                      onChange={(e) => updateStatus(viewItem.id, e.target.value as ItemStatus)}
                ...
                <div className="flex items-center gap-3 bg-slate-50 rounded-xl p-3 mb-4">
                  <div className="relative w-11 h-11 rounded-lg overflow-hidden bg-white shrink-0">
                    <Image
                      src={viewItem.product.images?.[0]?.url ?? viewItem.productImage}
                      alt={viewItem.product.name} fill className="object-cover" sizes="44px"
                    />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-slate-900 truncate">{viewItem.product.name}</p>
                    <p className="text-xs text-slate-400">
                      Qty: {viewItem.quantity}{viewItem.variant ? ` · ${viewItem.variant}` : ''}
                    </p>
                  </div>
                  <p className="text-sm font-black text-slate-900 shrink-0">
                    ${(Number(viewItem.priceAtTime) * viewItem.quantity).toFixed(2)}
                  </p>
                </div>
                ...
                <div className="bg-slate-50 rounded-xl p-4 space-y-1 text-sm">
                  ...
                  <p className="text-slate-400 text-xs">{viewItem.order.user.email}</p>
                  <p className="text-xs text-slate-400 mt-2">
                    Ordered {new Date(viewItem.order.createdAt).toLocaleDateString('en-US', {
